...
class Dashboardmodel extends CI_Model{	
	...
	public function borrower_advanced_info($uid) {
		
		$this->db->select('id');
		$this->db->from('borrowers_details_table');
		$this->db->where('user_id',$uid);
		$query = $this->db->get();
		if($this->db->affected_rows()>0)
		{
	...
	public function lender_advanced_info($uid) {
		
		$this->db->select('id');
		$this->db->from('lenders_details_table');
		$this->db->where('user_id',$uid);
		$query = $this->db->get();
		if($this->db->affected_rows()>0)
		{
	...
	public function fc_advanced_info($uid) {
		
		$this->db->select('id');
		$this->db->from('fc_details_table');
		$this->db->where('user_id',$uid);
		$query = $this->db->get();
		if($this->db->affected_rows()>0)
		{
	...
	public function borrower_advanced_info_updated() {
		...
		$arr['fc_id']=$this->session->userdata('user_id');
		$arr['borrower_id']=$this->session->userdata('user_id');
		
		$arr['date_added']=date("Y-m-d H:i:s");
		$arr['date_modified']=date("Y-m-d H:i:s");
		
		$query = $this->db-> insert('borrowers_details_table',$arr);
		if($this->db->affected_rows()>0)
		{
			$arry=array();
			$arry['user_id'] 		=$this->session->userdata('user_id');
			$arry['activity'] 		="Update Advanced Info Borrower";
			$arry['activity_table'] ="borrowers_details_table";
			$arry['datecreated'] 	=date("Y-m-d H:i:s");
			$query = $this->db-> insert('history',$arry);
			return true;
		}
	...
	public function lender_advanced_info_updated() {
		...
		$query = $this->db-> insert('lenders_details_table',$arr);
		if($this->db->affected_rows()>0)
		{
			$arry=array();
			$arry['user_id'] 		=$this->session->userdata('user_id');
	...
	public function fc_advanced_info_updated() {
		...
		$query = $this->db-> insert('fc_details_table',$arr);
		if($this->db->affected_rows()>0)
		{
			$arry=array();
			$arry['user_id'] 		=$this->session->userdata('user_id');
