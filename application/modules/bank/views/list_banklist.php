              ...
              <table id="demo-foo-addrow" class="table m-t-30 table-hover contact-list" data-page-size="10">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><?=$status;?></td>
                    <td><a href="<?=base_url();?>bank/edit_bank/<?=$row['bank_id'];?>" style="background-position: 0 -118px;"><button type="button" class="btn btn-sm btn-icon btn-pure btn-outline" title="Edit"><i class="ti-edit" aria-hidden="true"></i>Edit</button></a> | <a href="<?=base_url();?>bank/delete_bank/<?=$row['bank_id'];?>" onclick="return confirm_delete()" style="background-position: 0 -78px;"><button type="button" class="btn btn-sm btn-icon btn-pure btn-outline" title="Delete">Delete</button></a></td>
                  </tr>
                  <? $i++;}}else
				  { ?>
                  <tr>
                  	<td colspan="9">No Bank Found!</td>
                  </tr>
                  <? }?>
                </tbody>
                <tfoot>
                  <tr>
                    <td colspan="2"><button type="button" class="btn btn-info btn-rounded" data-toggle="modal" data-target="#myModal">Add New Bank</button></td>
                      <div id="myModal" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                        <form action="<?=base_url();?>bank/add_banks/" method="post" novalidate="novalidate" onsubmit="return add_role_valid()">
